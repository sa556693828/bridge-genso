...
bridgeEthListener.events.SwapInitialized().on("data", async (event: any) => {
...
bridgeMumListener.events.SwapInitialized().on("data", async (event: any) => {
  const [acc0] = await ethers.getSigners();
  const { from, to, amount, nonce, chainId, symbol } = event.returnValues;
  const messageHash = await signMessage(
    acc0.address,
...
bridgeBscListener.events.SwapInitialized().on("data", async (event: any) => {
  const [acc0] = await ethers.getSigners();
  const { from, to, amount, nonce, chainId, symbol } = event.returnValues;
  const messageHash = await signMessage(
    acc0.address,
